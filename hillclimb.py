...
if __name__ == "__main__":
    ...
    # rds = Rediscovery("C12=CC(C=CC3=C4C=C5C(C(C=C6C(C=C7)=C8)=C8C=C5)=C3)=C4C=C1C9=CC(C(C%10=C%11)=CC%12=C%11C=CC%13=CC%14=C(C=C%13%12)C=CC%15=C%14C=C(C%16=CC(C(C%17=C%18)=CC%19=C%18C=CC%20=CC%21=C(C=C%20%19)C=CC%22=C%21C=C(C%23=CC6=C7C=C%23C=C%24)C%24=C%22)=C(C=C%17)C=C%16C=C%25)C%25=C%15)=C(C=C%10)C=C9C=C2") # MöbiusCNB
    ...
    for k in [True,False]:
        for j in [[10]]:
            ...
            for i in range(10):
                ...
                df.to_csv(f'data/rediscovery/hillclimb_rediscovery-perylene{"_isEZ" if is_EZ else ""}_{"comp" if k else "unit"}_{i+42}.csv', index=False)
